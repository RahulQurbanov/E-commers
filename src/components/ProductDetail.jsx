// import React, { useEffect, useState } from "react";
// import { useParams } from "react-router-dom";
// import ReactImageMagnify from "react-image-magnify";

// export default function ProductDetail() {
//   const { id } = useParams();
//   const [product, setProduct] = useState(null);
//   const [loading, setLoading] = useState(true);

//   // Ürün detaylarını API'den al
//   async function fetchProductDetail() {
//     try {
//       let response = await fetch(`https://test.mybrands.az/api/v1/products/${id}`);
//       if (!response.ok) {
//         throw new Error("Ürün bilgisi alınamadı.");
//       }
//       let data = await response.json();
//       setProduct(data);
//       setLoading(false);
//     } catch (error) {
//       console.error("Hata:", error);
//       setLoading(false);
//     }
//   }

//   useEffect(() => {
//     fetchProductDetail();
//   }, [id]);

//   if (loading) {
//     return <p>Yükleniyor...</p>;
//   }

//   if (!product) {
//     return <p>Ürün bilgisi bulunamadı.</p>;
//   }

//   return (
//     <div className="w-[79%] m-auto p-4 flex flex-col">
//       <div className="flex gap-32">
//         {/* Ürün Resimleri */}
//         <div className="flex gap-7 w-[42%] m-auto">
//           {/* Küçük Resimler */}
//           <div className="w-[85px] flex flex-col gap-2 cursor-pointer">
//             {product.image.items.map((img, index) => (
//               <img
//                 key={index}
//                 src={`https://test.mybrands.az${img.file}`}
//                 alt={`Thumbnail ${index + 1}`}
//                 className="w-full h-auto"
//               />
//             ))}
//           </div>

//           {/* Büyük Resim - Yakınlaştırılabilir */}
//           <div className="w-[500px]">
//             <ReactImageMagnify
//               {...{
//                 smallImage: {
//                   alt: "Main Product",
//                   isFluidWidth: true,
//                   src: `https://test.mybrands.az${product.image.items[0].file}`,
//                 },
//                 largeImage: {
//                   src: `https://test.mybrands.az${product.image.items[0].file}`,
//                   width: 1200,
//                   height: 1000,
//                 },
//                 lensStyle: { backgroundColor: "rgba(255, 255, 255, 0.3)" },
//                 enlargedImageContainerDimensions: {
//                   width: "150%",
//                   height: "100%",
//                 },
//               }}
//             />
//           </div>
//         </div>

//         {/* Ürün Detayları */}
//         <div className="mt-6 flex flex-col items-center gap-6 w-[58%] font-circe">
//           <div className="flex flex-col gap-[1px]">
//             <span className="text-2xl font-bold">{product.product.title_az}</span>
//             <span className="text-[13px] text-gray-400 text-center font-light">
//               {product.category}
//             </span>
//           </div>
//           <div className="flex flex-col gap-2">
//             <span className="text-center text-xl font-bold">${product.price}</span>
//             <a href="#" className="underline text-[12px]">
//               Çatdırılma və geri qaytarılma haqqında məlumat
//             </a>
//           </div>
//           <div className="flex flex-col w-[90%] gap-[20px]">
//             <button className="bg-[#212D4A] text-white py-3 px-7 text-xl">
//               Səbətə əlavə et
//             </button>
//             <button className="text-[#212D4A] bg-gray-100 py-3 px-7 text-xl">
//               <i className="fa-regular fa-heart pr-4"></i>Arzu olunanlara əlavə et
//             </button>
//           </div>
//         </div>
//       </div>

//       {/* Alt Bilgiler */}
//       <div className="w-[100%] flex items-center gap-5 mt-20">
//         <div className="border-[1px] border-[#e0e0e0] w-[32%] px-12 py-5">
//           <h1 className="text-[15px] font-bold mb-4">MƏHSUL HAQQINDA ƏSAS MƏLUMAT</h1>
//           <p className="text-[13px] mb-5">Məhsulun kodu: {product.product.code}</p>
//           <div className="text-[13px]">
//             <p>- {product.category}</p>
//             <p>- Çanta və aksesuarlar</p>
//             <p>- {product.product.gender}</p>
//           </div>
//         </div>
//         <div className="border-[1px] border-[#e0e0e0] w-[32%] h-[250px] px-12 py-5">
//           <h1 className="text-[15px] font-bold mb-5">MƏHSUL HAQQINDA ƏTRAFLI MƏLUMAT</h1>
//         </div>
//         <div className="border-[1px] border-[#e0e0e0] w-[30%] px-12 py-5 h-[250px]">
//           <h1 className="text-[15px] font-bold mb-5">Seçilmiş parametrlər</h1>
//           <div className="text-[14px]">
//             <p>- Ölçü: {product.size || "no size"}</p>
//             <p>- Rəng: {product.color || "no color"}</p>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// }


import React, { useEffect, useState } from "react";
import { useParams } from "react-router-dom";
import ReactImageMagnify from "react-image-magnify";

export default function ProductDetail() {
  const { id } = useParams();
  const [product, setProduct] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchProductDetail() {
      try {
        const response = await fetch(`https://test.mybrands.az/api/v1/products/${id}`);
        if (!response.ok) throw new Error("Ürün bilgisi alınamadı.");
        const data = await response.json();
        setProduct(data);
      } catch (error) {
        console.error("Hata:", error);
      } finally {
        setLoading(false);
      }
    }
    fetchProductDetail();
  }, [id]);

  if (loading) return <p>Yüklənir...</p>;
  if (!product) return <p>Məhsul tapılmadı.</p>;

  return (
    <div className="w-[85%] m-auto flex flex-col p-6">
      <div className="flex gap-10">
        {/* Kiçik Şəkillər */}
        <div className="w-[20%] flex flex-col gap-2">
          {product.image.items.map((img, index) => (
            <img
              key={index}
              src={`https://test.mybrands.az${img.file}`}
              alt={`Thumbnail ${index}`}
              className="w-full cursor-pointer"
            />
          ))}
        </div>

        {/* Böyük Şəkil */}
        <div className="w-[50%]">
          <ReactImageMagnify
            {...{
              smallImage: {
                alt: "Product Image",
                isFluidWidth: true,
                src: `https://test.mybrands.az${product.image.items[0].file}`,
              },
              largeImage: {
                src: `https://test.mybrands.az${product.image.items[0].file}`,
                width: 1200,
                height: 800,
              },
            }}
          />
        </div>
      </div>

      {/* Məhsul Məlumatı */}
      <div className="mt-10">
        <h1 className="text-2xl font-bold">{product.product.title_az}</h1>
        <p className="text-gray-600">{product.category}</p>
        <p className="text-lg font-bold mt-2">${product.price}</p>
      </div>
    </div>
  );
}
